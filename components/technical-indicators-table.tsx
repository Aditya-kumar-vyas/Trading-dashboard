...
export default function TechnicalIndicatorsTable({
  ...
  const processData = (rawData: OHLCData[]): IndicatorData[] => {
    ...
    // Ensure timestamps are properly aligned to exact minutes for consistency
    const normalizedData = sortedData.map((candle) => {
      const candleTime = new Date(candle.timestamp);
      // Ensure each candle timestamp is set to the exact minute (xx:xx:00.000)
    ...
    const ema9Values = calculateEMA(normalizedData, 9);
    const ema18Values = calculateEMA(normalizedData, 18);
    ...
    const dataWithIndicators: IndicatorData[] = normalizedData.map(
      (candle, index) => ({
        ...candle,
  ...
  useEffect(() => {
    ...
    // Setup a special interval that triggers exactly at the start of each minute
    const setupExactMinuteInterval = () => {
      const now = new Date();
      const secondsToNextMinute = 60 - now.getSeconds();
      const msToNextMinute = secondsToNextMinute * 1000 - now.getMilliseconds();

      debugLog(
        `Setting up exact minute timer - will fire in ${msToNextMinute}ms`
      );

      // Set timeout to run exactly at the next minute boundary
      setTimeout(() => {
        debugLog("Exact minute interval triggered!");

        // Update data precisely at the minute boundary
        if (isConnected) {
          updateWithRealTimeData();
        }

        // Then set up regular 1-minute intervals precisely aligned with clock minutes
        const minuteInterval = setInterval(() => {
          debugLog("Exact minute interval fired");
          if (isConnected) {
            updateWithRealTimeData();
          }
        }, 60000); // Every minute
        ...
        updateIntervalRef.current = minuteInterval;
      }, msToNextMinute);
    };

    // Set up the exact minute interval
    setupExactMinuteInterval();

    // Also set up a faster polling interval (200ms) to ensure price updates within each minute
  ...
  const initializeCurrentCandle = (time: Date, price: number) => {
    // Ensure the timestamp is at an exact minute boundary
    const exactMinuteTime = new Date(time);
    exactMinuteTime.setSeconds(0);
    exactMinuteTime.setMilliseconds(0);
